...
import Midi.DeltaTime (DeltaTime)
...
bytes (NoteOn note) =
  [ 0x90                -- 0x9_ = "Note on", 0x_0 = channel 0
  , convert note        -- note
  , 0 ]                 -- max velocity

bytes (NoteOff note) =
  [ 0x80                -- 0x8_ = "Note off", 0x_0 = channel 0
  , convert note        -- note
  , 0 ]                 -- max velocity

bytes (SetKeySignature key scale) =
  [ 0xff                -- meta event
...
bytes (SetTimeSignature numer denom) =
  [ 0xff                -- meta event
  ...
  -- denominator is input a power of 2
  where denom' = floor $ logBase (fromIntegral 2) (fromIntegral denom)

putsEvent :: Event -> Put
putsEvent e = forM_ (bytes e) putWord8
